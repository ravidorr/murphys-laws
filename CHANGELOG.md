...
### Added
- Work Sans variable font bundled in the Android app for visual parity with web. [android/app/src/main/res/font/work_sans_variable.ttf](android/app/src/main/res/font/work_sans_variable.ttf) (~360 KB, single VF covers all weights via the `wght` axis), wrapped by a hand-authored `WorkSans` `FontFamily` in [android/app/src/main/java/com/murphyslaws/ui/theme/WorkSans.kt](android/app/src/main/java/com/murphyslaws/ui/theme/WorkSans.kt). The FontFamily registers the variable font at four weight slots (Normal 400, Medium 500, SemiBold 600, Bold 700) with `FontVariation.Settings(FontVariation.weight(...))` so Compose's typography system resolves `.weight(...)` requests to the correct axis position. Requires API 26+, which matches the app's `minSdk`. [web/scripts/export-android-tokens.ts](web/scripts/export-android-tokens.ts) now emits `fontFamily = WorkSans` for every `DS.Typography.<level>`, replacing the `FontFamily.Default` (Roboto) placeholder from PR Android-1. PR Android-3 (final phase) of the design-tokens-mobile rollout. Bundling a new font is user-visible, so Android app bumped MINOR to `1.1.0` / versionCode `4`; root to `2.4.21`.

### Changed
- Android screens now consume `DS.Color.*` for vote thumbs and the markdown link colour: vote-up `Color(0xFF10b981)` -> `DS.Color.success`, vote-down `Color(0xFFef4444)` -> `DS.Color.error` across [SearchScreen.kt](android/app/src/main/java/com/murphyslaws/presentation/search/SearchScreen.kt), [LawDetailScreen.kt](android/app/src/main/java/com/murphyslaws/presentation/lawdetail/LawDetailScreen.kt), [HomeScreen.kt](android/app/src/main/java/com/murphyslaws/presentation/home/HomeScreen.kt), [LawListCard.kt](android/app/src/main/java/com/murphyslaws/presentation/components/LawListCard.kt); markdown links `Color(0xFF1976D2)` -> `DS.Color.btnPrimaryBg` in [MarkdownParser.kt](android/app/src/main/java/com/murphyslaws/util/MarkdownParser.kt); RiskLevel LOW/HIGH `Color.Green` / `Color.Red` -> `DS.Color.success` / `DS.Color.error` in [RiskLevel.kt](android/app/src/main/java/com/murphyslaws/domain/model/RiskLevel.kt). RiskLevel MEDIUM stays as `Color(0xFFFFA500)` with a comment because no DS token matches the bright friendly orange affordance (DESIGN.md's `orange-text` is the dark-brown calculator-pair foreground, not a chip indicator). Third-party brand colours for the social-share icons (X, Facebook, LinkedIn, Reddit, Email) intentionally stay inline; they aren't part of the Murphy's Laws design system. PR Android-2 of the design-tokens-mobile rollout. Android app bumped to `1.0.2` / versionCode `3`; root to `2.4.20`.

### Added
- Android design-token codegen. New [web/scripts/export-android-tokens.ts](web/scripts/export-android-tokens.ts) (wired as `npm --prefix web run design:export:android`) reads `shared/DESIGN.md` and emits:
  - `android/app/src/main/java/com/murphyslaws/ui/theme/Tokens.kt` -- `object DS` with nested `Color`, `Spacing`, `Radius`, `Typography`. Each color pair exposes both light (`bg`) and the actual DESIGN.md dark counterpart (`darkBgPrimary`) as separate vals so `Theme.kt` can plug them into Compose's `lightColorScheme` / `darkColorScheme`. Naming dark vals after the actual dark token (instead of the simple `dark<LightName>` shape) avoids collisions with standalone `dark-*` tokens like `dark-bg`. Inner `object Color` uses the fully-qualified `androidx.compose.ui.graphics.Color` so it doesn't shadow the Compose Color class.
  - `android/app/src/main/res/values/colors.xml` and `android/app/src/main/res/values-night/colors.xml` -- `ds_<token>` resources for any non-Compose XML consumer; the day/night auto-swap is driven by the resource system.
  - `android/app/src/main/res/values/dimens.xml` -- `ds_space_<n>` and `ds_radius_<level>` dp resources.

  **Generated outputs are gitignored** and produced by an `exportDesignTokens` Gradle task (added to [android/app/build.gradle.kts](android/app/build.gradle.kts) and wired into `preBuild`), and on demand via `npm --prefix web run design:export:android`. There's no committed baseline to drift from, which is why `--check` is a smoke test (verify the generator runs, returns non-empty `Tokens.kt` and at least one `ds_*` color resource) rather than a diff comparison. Typography uses `FontFamily.Default` (Roboto) until Work Sans is bundled (PR Android-3). The DARK_PAIR_OVERRIDES table mirrors the iOS exporter: `bg`->`dark-bg-primary`, `success-text`->`dark-success-fg`, etc., plus `dark-bg`->`dark-dark-bg` and `dark-text`->`dark-dark-fg` so the calc-dark "dark emphasis" surface gets a `values-night/colors.xml` override. 21 vitest cases. Android app bumped to `1.0.1` / versionCode `2`; root to `2.4.19`. `web/package.json` not bumped (per ship skill: tooling-only diffs in `web/scripts/` are not a user-facing web change).

### Changed
- Android theme rewritten to consume the generated DS tokens. [android/app/src/main/java/com/murphyslaws/ui/theme/Theme.kt](android/app/src/main/java/com/murphyslaws/ui/theme/Theme.kt) now sources every `MaterialTheme.colorScheme` slot from `DS.Color.*`; M3 slot mapping recorded in a comment block. The drifted `Color.kt` (which had `WebLightPrimary = #1173D4` while DESIGN.md says `btn-primary-bg = #0d5ea1`, and the inferred `WebLightSurface = #F3F4F6` not in DESIGN.md at all) is deleted; its constants weren't referenced outside Theme.kt. `surfaceVariant` is no longer overridden because DESIGN.md doesn't tokenize a tinted-surface colour; M3's default is acceptable. [android/app/src/main/java/com/murphyslaws/ui/theme/Type.kt](android/app/src/main/java/com/murphyslaws/ui/theme/Type.kt) maps M3 typography slots (displayLarge, headlineLarge, titleLarge, bodyLarge, labelSmall, etc.) to `DS.Typography.<level>`; the standing `Roboto for now` TODO is removed. Unblocks PR Android-2 which will replace inline `Color(0xFF...)` references in screen files.
