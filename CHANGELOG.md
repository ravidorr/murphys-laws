...
### Added
- Work Sans variable font bundled in the iOS app for visual parity with web. [ios/MurphysLaws/Resources/Fonts/WorkSans-VariableFont_wght.ttf](ios/MurphysLaws/Resources/Fonts/WorkSans-VariableFont_wght.ttf) (~360 KB, single VF covers all weights via the `wght` axis), registered via `UIAppFonts` in [Info.plist](ios/MurphysLaws/Info.plist), and shipped through the XcodeGen-managed source tree. [web/scripts/export-ios-tokens.ts](web/scripts/export-ios-tokens.ts) now emits `Font.custom("WorkSans-Regular", size:).weight(...)` for every `DS.Typography.<level>` so SwiftUI picks up the bundled font by its PostScript name. Resolves the placeholder `Font.system(size:)` from PR iOS-1. PR iOS-3 of the design-tokens-mobile rollout. Bundling a new font is a user-visible change, so iOS app bumped MINOR to `1.1.0` / build `4`; root to `2.4.19`.
