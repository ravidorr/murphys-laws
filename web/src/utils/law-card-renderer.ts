...
import { firstAttributionLine } from './attribution.ts';
import { escapeHtml, highlightSearchTerm } from './sanitize.ts';
import { getUserVote } from './voting.ts';
 ...
 */
interface LawCardOptions {
  index?: number;
  rankOffset?: number | null;
  searchQuery?: string;
}
...
export function renderLawCard(law: any, options: LawCardOptions = {}) {
  ...
  const shareButtonsHtml = renderShareButtonsHTML({
    lawId: safeId,
    lawText: law.text || '',
    url: `${window.location.origin}/law/${safeId}`
  });
