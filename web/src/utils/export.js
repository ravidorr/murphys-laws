 ...
 * import { getExportContent } from '../utils/export-context.js';
 *
 ...
 */
export function exportToCSV(content, filename) {
  const { type, title, data } = content;
  let csv = '';
 ...
 */
function getLawDisplayText(law) {
 ...
 * @param {string} [filename] - Optional filename
 */
export function exportToMarkdown(content, filename) {
  const { type, title, data } = content;
  let md = `# ${title}\n\n`;
 ...
 * @param {string} [filename] - Optional filename
 */
export function exportContent(content, format, filename) {
  switch (format) {
    case 'pdf':
