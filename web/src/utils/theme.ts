...
import type { Theme, EffectiveTheme } from '../types/app.d.ts';
...
export function getTheme(): Theme {
  ...
  try {
    ...
    return VALID_THEMES.includes(stored as Theme) ? stored as Theme : 'auto';
  } catch {
 ...
 */
export function getThemeTooltip(theme?: Theme): string {
  const currentTheme = theme ?? getTheme();
  switch (currentTheme) {
    case 'light':
      return 'Light mode';
