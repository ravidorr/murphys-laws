...
- [x] **User Favorites/Bookmarks:** ~~Allow users to save favorite laws using localStorage. Show favorites count and a dedicated favorites page.~~ Done - added bookmark button to law cards (toggles favorite state), dedicated `/favorites` page with empty state, localStorage persistence. Favorite button uses a single filled bookmark icon for both add and remove; state is shown by styling (.favorited) and tooltip. Feature controlled by `VITE_FEATURE_FAVORITES` env var or localStorage admin override (`murphys_ff_favorites`).
