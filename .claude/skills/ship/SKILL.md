...
Two files always bump together, independently of each other's scheme:

| File | Scheme | Example |
|------|--------|---------|
| `package.json` (root) | `MAJOR.MINOR.PATCH` | `2.1.1` → `2.1.2` |
| `web/package.json` | `MAJOR.MINOR.PATCH` | `3.1.35` → `3.1.36` |

Increment only the **PATCH** segment of each unless the change warrants MINOR (new user-facing feature) or MAJOR (breaking change). Bump both files even if only one sub-package changed.

`backend/package.json` has its own version (`2.0.x`) — only bump it when backend code actually changed.

> **Why both?** The root version drives the Sentry `release` tag; the web version is embedded in built assets. Keeping them in sync avoids confusion in Sentry dashboards.
